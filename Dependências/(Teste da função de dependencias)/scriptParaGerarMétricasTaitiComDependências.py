...
path_of_the_directory = r"F:\Pesquisa TAITI\Dependências\(Teste da função de dependencias)"
degreeofdependence= 30
df = pd.read_csv('https___github.com_oneclickorgs_one-click-orgs.csv',engine="python", sep=';')
...
for filename in sorted(os.listdir(path_of_the_directory), key = natural_keys):
    if filename.endswith('.csv') and not filename.startswith("http") and (pd.read_csv(filename, sep=",")).values.tolist() == []:
            ...
            index = index+1
            continue
    elif filename.endswith('.csv') and not filename.startswith("http"):
...
df.to_excel('TaitiWithdeps_one-click-orgs-Modificado.xlsx', index=False)
