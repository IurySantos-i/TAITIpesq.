...
for type in degree:
    ...
    for filename in sorted(os.listdir(path_of_the_directory), key = natural_keys):
        if filename.endswith(degree) and not filename.startswith("http") and (pd.read_csv(filename, sep=";")).values.tolist() == []:
        ...
        if filename.endswith(degree) and not filename.startswith("http"):
    ...
    df.to_csv('TaitiWithdeps_' + repositorio + degree, index=False)
